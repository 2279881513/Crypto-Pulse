...
| 功能 | 说明 |
|------|------|
| **多因子评分** | EMA/MACD/RSI/布林带/OBV/ADX/成交量 + 多周期共振 |
| **止盈止损** | 止盈 = 入场价 + n × ATR（n=3.0），止损 = ATR × 2.0 |
| **止损冷却** | 止损触发后暂停交易5分钟，冷却期内不开新仓 |
| **保本检查** | 进场前判断TP1利润是否高于双边手续费，不够则放弃 |
| **风控标记** | 被风控阻止的信号以橙色箭头标记在K线图上 |
| **风控统计** | 底部面板显示风控原因分布，可点击高亮对应K线 |
| **仓位管理** | 本金、杠杆、费率可调，自动计算净利、ROI、手续费 |
| **保本过滤** | 自动过滤利润不够扣手续费的低质量交易 |
| **实时模拟** | 加载历史数据预热指标，从现在开始模拟交易 |
| **评分分布** | 按评分段统计准确率，离场原因分布含超时盈利/亏损细分 |
| **交易明细** | 点击某条记录自动跳转到对应K线并闪烁标记 |
| **数据导出** | Excel格式(.xls)，含列宽设置，文件名精确到秒 |
...
| 参数 | 值 | 说明 |
|------|-----|------|
| 止盈倍数 n | 3.0 | 止盈 = 入场价 ± n × ATR |
| 止损倍数 | 2.0 | 止损 = 入场价 ± 2.0 × ATR |
| 验证值 | 5~30（可调） | 入场后检查多少根K线 |

### 风控规则

| 规则 | 说明 |
|------|------|
| 止损冷却 | 止损触发后暂停交易5分钟，期间不开新仓 |
| 保本检查 | TP1利润 > 双边手续费才进场，否则放弃并标记 |
| 同方向冷却 | 同方向连续信号间隔 ≥ 2根K线（ADX>50+BB触边豁免） |
| 强趋势方向保护 | ADX>45 且方向与EMA趋势相反时强制观望 |
| 极端趋势保护 | ADX>80 强制观望不开新仓 |
